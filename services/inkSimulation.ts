...
export class InkSimulation {
    ...
    step() {
        ...
        for (let i = 0; i < size; i++) {
            ...
            const relaxVal = 1.0 - this.omega;
            const omega = this.omega;

            // k=0
            let val0 = f0 * relaxVal + omega * c0;
            if (rho > 1.0) val0 *= evap;
            this.f[baseIdx + 0] = val0;

            // Axis directions (weight 1/9)
            let feq, val;
            
            feq = w1_rho * (1 + 3*ux + 4.5*ux*ux + uFactor);
            val = f1 * relaxVal + omega * feq;
            if (rho > 1.0) val *= evap;
            this.f[baseIdx + 1] = val;

            feq = w1_rho * (1 + 3*uy + 4.5*uy*uy + uFactor);
            val = f2 * relaxVal + omega * feq;
            if (rho > 1.0) val *= evap;
            this.f[baseIdx + 2] = val;

            const nux = -ux;
            feq = w1_rho * (1 + 3*nux + 4.5*nux*nux + uFactor);
            val = f3 * relaxVal + omega * feq;
            if (rho > 1.0) val *= evap;
            this.f[baseIdx + 3] = val;

            const nuy = -uy;
            feq = w1_rho * (1 + 3*nuy + 4.5*nuy*nuy + uFactor);
            val = f4 * relaxVal + omega * feq;
            if (rho > 1.0) val *= evap;
            this.f[baseIdx + 4] = val;

            // Diagonal directions (weight 1/36)
            let eu;
            
            eu = ux + uy;
            feq = w2_rho * (1 + 3*eu + 4.5*eu*eu + uFactor);
            val = f5 * relaxVal + omega * feq;
            if (rho > 1.0) val *= evap;
            this.f[baseIdx + 5] = val;

            eu = -ux + uy;
            feq = w2_rho * (1 + 3*eu + 4.5*eu*eu + uFactor);
            val = f6 * relaxVal + omega * feq;
            if (rho > 1.0) val *= evap;
            this.f[baseIdx + 6] = val;

            eu = -ux - uy;
            feq = w2_rho * (1 + 3*eu + 4.5*eu*eu + uFactor);
            val = f7 * relaxVal + omega * feq;
            if (rho > 1.0) val *= evap;
            this.f[baseIdx + 7] = val;

            eu = ux - uy;
            feq = w2_rho * (1 + 3*eu + 4.5*eu*eu + uFactor);
            val = f8 * relaxVal + omega * feq;
            if (rho > 1.0) val *= evap;
            this.f[baseIdx + 8] = val;
        }
        ...
        for (let i = 0; i < size; i++) {
            ...
            const fib = fibers[i];
            const absorb = adsorptionRate * (0.2 + fib);
            ...
            if (destX >= 0 && destX < w_1 && destY >= 0 && destY < h_1) {
                ...
                tempC[idx + 1] += cRem * w10; tempM[idx + 1] += mRem * w10; tempY[idx + 1] += yRem * w10;
                tempC[idx + w] += cRem * w01; tempM[idx + w] += mRem * w01; tempY[idx + w] += yRem * w01;
                tempC[idx + w + 1] += cRem * w11; tempM[idx + w + 1] += mRem * w11; tempY[idx + w + 1] += yRem * w11;
            } else {
